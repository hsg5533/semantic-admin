      ...
      module = {
        ...
        open: function (index) {
          ...
          if (!$activeContent.is(":animated")) {
            ...
            if (settings.exclusive && contentIsOpen) {
              $previousTitle.removeClass(className.active);
              $previousContent
                .stop()
                .children()
                .animate(
        ...
        close: function (index) {
          ...
          $activeContent
            ...
            .end()
            .slideUp(settings.duration, settings.easing, function () {
              $activeContent.attr("style", "").removeAttr("style");
            ...
            $.each(query, function (depth, value) {
              var camelCaseValue =
                depth != maxDepth
                  ...
                  : query;
              if (
                $.isPlainObject(instance[camelCaseValue]) &&
                depth != maxDepth
              ) {
...
(function ($, window, document, undefined) {
  $.api = $.fn.api = function (parameters) {
    var settings = $.extend(true, {}, $.api.settings, parameters),
      ...
      context = typeof this != "function" ? this : $("<div/>"),
      // context defines the element used for loading/error state
      $context = settings.stateContext ? $(settings.stateContext) : $(context),
    ...
    module = {
      initialize: function () {
          ...
          .done(function (response) {
            ...
            if (settings.dataType == "json") {
              if (response.error !== undefined) {
                $.proxy(settings.failure, $context)(
                  response.error,
                  settings,
                  $module
                );
          ...
          .fail(function (xhr, status, httpMessage) {
            ...
            if (xhr !== undefined) {
              ...
              if (xhr.readyState !== undefined && xhr.readyState == 4) {
                ...
                module.debug(
                  "Request Aborted (Most likely caused by page change)"
                );
              }
            }
          });
              ...
              xhr.upload.addEventListener(
                ...
                function (event) {
                  ...
                  if (event.lengthComputable) {
                    percentComplete =
                      Math.round((event.loaded / event.total) * 10000) / 100 +
                      "%";
                    $.proxy(settings.progress, $context)(
                      percentComplete,
                      event
                    );
                  }
        ...
        xhr = $.ajax(ajaxSettings)
          .always(function () {
            ...
            loadingDelay =
              settings.loadingLength - (new Date().getTime() - loadingTimer);
            settings.loadingDelay = loadingDelay < 0 ? 0 : loadingDelay;
          })
          .done(function (response) {
            var context = this;
            setTimeout(function () {
        ...
        if (settings.stateContext) {
          $module.data(metadata.promise, promise).data(metadata.xhr, xhr);
        }
      },
      ...
      get: {
        ...
        templateURL: function () {
          var action =
              $module.data(settings.metadata.action) ||
              settings.action ||
              false,
            url;
          if (action) {
            ...
            if (urlVariables) {
              ...
              $.each(urlVariables, function (index, templateValue) {
                var term = templateValue.substr(2, templateValue.length - 3),
                  termValue =
                    $.isPlainObject(urlData) && urlData[term] !== undefined
                      ? urlData[term]
      ...
      // reset api request
      reset: function () {
        $module.data(metadata.promise, false).data(metadata.xhr, false);
  ...
  $.fn.apiButton = function (parameters) {
    $(this).each(function () {
      var // if only function passed it is success callback
        $module = $(this),
        selector = $(this).selector || "",
        settings = $.isFunction(parameters)
          ? $.extend(true, {}, $.api.settings, $.fn.apiButton.settings, {
              stateContext: this,
              success: parameters,
            })
          : $.extend(
      ...
      module = {
        initialize: function () {
          if (settings.context && selector !== "") {
            ...
            $module.on("click." + settings.namespace, module.click);
          }
        },
        click: function () {
          if (
            !settings.filter ||
            $(this).filter(settings.filter).size() === 0
          ) {
...
(function ($, window, document, undefined) {
    ...
    $(this).each(function (instanceIndex) {
      ...
      module = {
        ...
        canvas: {
          ...
          merge: function () {
            ...
            mainContext.putImageData(
              imageContext.getImageData(0, 0, width, height),
              0,
              0
            );
        ...
        invoke: function (methodName, context, methodArguments) {
          var method;
          methodArguments =
            methodArguments || Array.prototype.slice.call(arguments, 2);
      ...
      if (instance !== undefined && moduleArguments) {
        ...
        return module.invoke(
          moduleArguments[0],
          this,
          Array.prototype.slice.call(moduleArguments, 1)
        );
      }
      // initializing
      module.initialize();
  ...
  $.fn.colorize.settings = {
    ...
    error: {
      ...
      undefinedHeight:
        "The width or height of image canvas could not be automatically determined. Please specify a height.",
    },
  };
})(jQuery, window, document);
    ...
    $allModules.each(function () {
      ...
      module = {
        initialize: function () {
          ...
          $field.on(
            module.get.changeEvent() + eventNamespace,
            module.event.field.change
          );
          module.instantiate();
        },
        ...
        get: {
          ...
          field: function (identifier) {
            ...
            if ($field.filter("#" + identifier).size() > 0) {
              ...
              $field.filter('[name="' + identifier + '"]').size() > 0
            ) {
              return $field.filter('[name="' + identifier + '"]');
            } else if (
              $field
                .filter("[data-" + metadata.validate + '="' + identifier + '"]')
                .size() > 0
            ) {
              return $field.filter(
        ...
        validate: {
          ...
          field: function (field) {
            ...
            if (field.rules !== undefined) {
              $.each(field.rules, function (index, rule) {
                if (
                  module.has.field(field.identifier) &&
          ...
          rule: function (field, validation) {
            ...
            if (bracket !== undefined && bracket !== null) {
              ...
              isValid = $.proxy(settings.rules[functionType], $module)(
                value,
                ancillary
              );
            }
    ...
    rules: {
      ...
      match: function (value, fieldIdentifier) {
        ...
        if ($form.find("#" + fieldIdentifier).size() > 0) {
          ...
          $form.find('[data-validate="' + fieldIdentifier + '"]').size() > 0
        ) {
          matchingValue = $form
            .find('[data-validate="' + fieldIdentifier + '"]')
            .val();
        }
...
(function ($, window, document, undefined) {
  $.fn.chatroom = function (parameters) {
    var $allModules = $(this),
      ...
      returnedValue;
    $(this).each(function () {
      var settings = $.extend(true, {}, $.fn.chatroom.settings, parameters),
        className = settings.className,
        namespace = settings.namespace,
        selector = settings.selector,
        error = settings.error,
      ...
      module = {
        ...
        initialize: function () {
          ...
          if (
            ...
            settings.channelName === undefined
          ) {
            module.error(error.key);
            return false;
          } else if (
            !(settings.endpoint.message || settings.endpoint.authentication)
          ) {
          ...
          $module.data("module", module).addClass(className.loading);
        },
        ...
        user: {
          ...
          joined: function (member) {
            ...
            if (member.id != "anonymous" && users[member.id] === undefined) {
              ...
              if (member.info.isAdmin) {
                $(html).prependTo($userList);
              } else {
                $(html).appendTo($userList);
              }
              if (settings.partingMessages) {
                $log.append(settings.templates.joined(member.info));
          ...
          left: function (member) {
            ...
            if (member !== undefined && member.id !== "anonymous") {
              ...
              if (settings.partingMessages) {
                $log.append(settings.templates.left(member.info));
                module.message.scroll.test();
              }
              module.user.updateCount();
            }
        ...
        event: {
          input: {
            keydown: function (event) {
              if (event.which == 13) {
                $messageButton.addClass(className.down);
              }
            },

            keyup: function (event) {
              if (event.which == 13) {
                $messageButton.removeClass(className.down);
                module.event.submit();
              }
            },
          },
          ...
          submit: function () {
            ...
            if (
              loggedInUser !== undefined &&
              !module.utils.emptyString(message)
            ) {
          ...
          toggleExpand: function () {
            if (!$module.hasClass(className.expand)) {
              $expandButton.addClass(className.active);
              module.expand();
            } else {
              $expandButton.removeClass(className.active);
              module.contract();
            }
          },
          ...
          toggleUserList: function () {
            if (!$log.is(":animated")) {
              if (!$userListButton.hasClass(className.active)) {
                $userListButton.addClass(className.active);
                module.user.list.show();
              } else {
                $userListButton.removeClass("active");
                module.user.list.hide();
              }
            }
          },
    ...
    return returnedValue ? returnedValue : this;
  };
        ...
        initialize: function () {
          ...
          if (settings.context && selector !== "") {
            ...
            $(element, settings.context)
              ...
              .on(
                selector + " + " + settings.selector.label,
                "click" + eventNamespace,
                module.toggle
              );
          } else {
      ...
      module = {
        ...
        event: {
          click: function (event) {
            ...
            if (
              $dimmer.find(event.target).size() === 0 ||
        ...
        is: {
          ...
          animating: function () {
            return (
              $dimmer.is(":animated") || $dimmer.hasClass(className.transition)
            );
          },
          dimmer: function () {
      ...
      module = {
        ...
        unbind: {
          intent: function () {
            ...
            if (hasTouch) {
              $document.off("touchstart" + eventNamespace);
            }
        ...
        event: {
          ...
          item: {
            ...
            click: function (event) {
              var $choice = $(this),
                text =
                  $choice.data(metadata.text) !== undefined
                    ? $choice.data(metadata.text)
                    : $choice.text(),
                value =
                  $choice.data(metadata.value) !== undefined
        ...
        determine: {
          ...
          intent: function (event, callback) {
            module.debug(
              "Determining whether event occurred in dropdown",
              event.target
            );
            callback = callback || function () {};
        ...
        set: {
          ...
          value: function (value) {
            module.debug(
              "Adding selected value to hidden input",
              value,
              $input
            );
            if ($input.size() > 0) {
          ...
          selected: function (value) {
            ...
            if ($selectedItem) {
              ...
              selectedText =
                $selectedItem.data(metadata.text) !== undefined
      ...
      module = {
        ...
        hideDimmer: function () {
          if ($dimmable.dimmer("is active")) {
            ...
            if (settings.closable) {
              $dimmer.off("click" + eventNamespace);
            }
...
(function ($, window, document, undefined) {
  $.fn.nag = function (parameters) {
    var $allModules = $(this),
      ...
      returnedValue;
    $(this).each(function () {
      var settings = $.extend(true, {}, $.fn.nag.settings, parameters),
        className = settings.className,
        selector = settings.selector,
        error = settings.error,
        namespace = settings.namespace,
      ...
      module = {
        initialize: function () {
          ...
          if (settings.context == window && settings.position == "fixed") {
            $module.addClass(className.fixed);
          }
          if (settings.sticky) {
        ...
        stick: function () {
          ...
          if (settings.position == "fixed") {
            var windowScroll =
                $(window).prop("pageYOffset") || $(window).scrollTop(),
              fixedOffset = $module.hasClass(className.bottom)
                ? contextOffset.top +
                  (contextHeight - moduleHeight) -
                  windowScroll
        ...
        should: {
          show: function () {
            ...
            if (
              module.storage.get(settings.storedKey) != settings.storedValue
            ) {
              module.debug(
                "Stored value is not set, can show nag",
                module.storage.get(settings.storedKey)
              );
              return true;
            }
            module.debug(
...
(function ($, window, document, undefined) {
  $.fn.popup = function (parameters) {
    ...
    $allModules.each(function () {
      var settings = $.isPlainObject(parameters)
        ...
        $window = $(window),
        $offsetParent = settings.inline ? $target.offsetParent() : $window,
        ...
        set: {
          position: function (position, arrowOffset) {
            var windowWidth = $(window).width(),
              ...
              offset = settings.inline ? $target.position() : $target.offset(),
              positioning,
              offstagePosition;
            position =
              position || $module.data(metadata.position) || settings.position;
            arrowOffset =
              arrowOffset || $module.data(metadata.offset) || settings.offset;
            // adjust for margin when inline
            if (settings.inline) {
              if (position == "left center" || position == "right center") {
                arrowOffset += parseInt(
                  window
                    .getComputedStyle(element)
                    .getPropertyValue("margin-top"),
                  10
                );
                distanceAway += -parseInt(
                  window
                    .getComputedStyle(element)
        ...
        bind: {
          close: function () {
            if (settings.on == "click" && settings.closable) {
              ...
              $document.on("click" + eventNamespace, module.hideGracefully);
            }
          },
        },
  ...
  $.fn.popup.settings = {
    ...
    error: {
      ...
      method: "The method you called is not defined.",
      recursion:
        "Popup attempted to reposition element to fit, but could not find an adequate position.",
    },
    ...
    $(this).each(function () {
      ...
      module = {
        initialize: function () {
          ...
          if (settings.automatic) {
            $prompt.on(inputEvent + eventNamespace, module.search.throttle);
          }
          $searchButton.on("click" + eventNamespace, module.search.query);
          $results.on(
            "click" + eventNamespace,
            selector.result,
            module.results.select
          );
          module.instantiate();
        },
        instantiate: function () {
        ...
        search: {
          ...
          query: function () {
            ...
            if (cachedHTML) {
              module.debug(
                "Reading result for '" + searchTerm + "' from cache"
              );
          ...
          local: function (searchTerm) {
            ...
            $.each(searchFields, function (index, field) {
              $.each(source, function (label, thing) {
                if (
                  typeof thing[field] == "string" &&
                  $.inArray(thing, results) == -1 &&
                  $.inArray(thing, fullTextResults) == -1
                ) {
        ...
        results: {
          generate: function (response) {
            ...
            var template = settings.templates[settings.type],
              html = "";
            if (
              ($.isPlainObject(response.results) &&
                !$.isEmptyObject(response.results)) ||
              ($.isArray(response.results) && response.results.length > 0)
            ) {
              if (settings.maxResults > 0) {
                response.results = $.makeArray(response.results).slice(
                  0,
                  settings.maxResults
                );
              }
          ...
          add: function (html) {
            if (
              settings.onResultsAdd == "default" ||
              $.proxy(settings.onResultsAdd, $results)(html) == "default"
            ) {
              $results.html(html);
            }
            module.results.show();
          ...
          select: function (event) {
            ...
            var $result = $(this),
              ...
              title = $title.html();
            if (
              settings.onSelect == "default" ||
              $.proxy(settings.onSelect, this)(event) == "default"
            ) {
    ...
    $allModules.each(function () {
      ...
      module = {
        ...
        is: {
          open: function () {
            return (
              $module.is(":animated") || $module.hasClass(className.active)
            );
          },
          closed: function () {
...
(function ($, window, document, undefined) {
  $.fn.tab = function (parameters) {
    var settings = $.extend(true, {}, $.fn.tab.settings, parameters),
      $module = $(this),
    ...
    module = {
          ...
          if (isTab) {
            ...
            if (isLastTab && remoteContent) {
              if (!shouldIgnoreLoad) {
                ...
                $.proxy(settings.onTabInit, $tab)(
                  currentPath,
                  parameterArray,
                  historyEvent
                );
                $.proxy(settings.onTabLoad, $tab)(
                  currentPath,
                  parameterArray,
                  historyEvent
                );
      ...
      get: {
        initialPath: function () {
          return (
            $module.eq(0).data(metadata.tab) || $tabs.eq(0).data(metadata.tab)
          );
        },
        path: function () {
        ...
        tabElement: function (tabPath) {
          ...
          return $fullPathTab.size() > 0 ? $fullPathTab : $simplePathTab;
        },
        tab: function () {
      ...
      module = {
        ...
        has: {
          direction: function (animation) {
            ...
            if (
              $module.hasClass(className.inward) ||
              $module.hasClass(className.outward)
            ) {
              return true;
            }
          },
        ...
        set: {
          ...
          direction: function () {
            if ($module.is(":visible")) {
              module.debug(
                "Automatically determining the direction of animation",
                "Outward"
              );
              $module.addClass(className.outward).removeClass(className.inward);
            } else {
              module.debug(
                "Automatically determining the direction of animation",
                "Inward"
              );
              $module.addClass(className.inward).removeClass(className.outward);
            }
          },
        ...
        get: {
          ...
          animationName: function () {
            ...
            for (animation in animations) {
              if (element.style[animation] !== undefined) {
                module.verbose(
                  "Determined animation vendor name property",
          ...
          animationEvent: function () {
            ...
            for (animation in animations) {
              if (element.style[animation] !== undefined) {
                module.verbose(
                  "Determined animation vendor end event",
                  animations[animation]
                );
                return animations[animation];
              }
      ...
      module = {
        ...
        generate: {
          ...
          html: function (source, id, url) {
            ...
            var width =
                settings.width == "auto" ? $module.width() : settings.width,
              height =
                settings.height == "auto" ? $module.height() : settings.height,
              html;
            if (source && id) {
              if (source == "vimeo") {
                html =
